(() => {
    ...
    if (!characterData.has(char))
      characterData.set(
        ...
        fetch(STROKE_SRC + encodeURIComponent(char) + ".json", {
          ...
          .then((r) => {
            if (!r.ok) throw Error("No stroke data");
            return r.json();
          })
          .catch((e) => {
            characterData.delete(char);
            throw e;
          }),
      );
    characterData.get(char).then(onComplete).catch(onError);
  }
  function loadFailure() {
    info(
      "Chưa có dữ liệu nét cho chữ này.",
      "Bạn vẫn có thể xem nghĩa và tra từ ghép.",
    );
  }
  function loadWriter() {
    ...
    writerPromise = new Promise((resolve, reject) => {
      ...
      s.onload = () =>
        window.HanziWriter
          ...
          : reject(new Error("HanziWriter unavailable"));
      s.onerror = () => reject(new Error("Cannot load HanziWriter"));
  ...
  function styleOnce() {
    if (document.getElementById("hanziPracticeStyle")) return;
    const st = document.createElement("style");
    st.id = "hanziPracticeStyle";
    st.textContent = `
...
`;
  ...
  function openModal(char, pinyin, mode) {
    ...
    const keydown = (e) => {
      ...
      if (e.key === "Tab") {
        const buttons = [...overlay.querySelectorAll("button,a[href]")].filter(
            (b) => !b.disabled,
          ),
          first = buttons[0],
          last = buttons.at(-1);
    ...
    overlay.querySelector(".hanzi-close").focus();
    overlay.querySelector(".hanzi-close").onclick = closeModal;
    overlay.addEventListener("click", (e) => {
      if (e.target === overlay) closeModal();
    });
    overlay
      .querySelectorAll(".hanzi-mode-btn")
      .forEach(
        (b) => (b.onclick = () => setupMode(char, pinyin, b.dataset.mode)),
      );
    overlay.querySelector("#hanziClear").onclick = () => clearDrawing();
    loadWriter()
      .then(() => {
        if (overlay.isConnected) setupMode(char, pinyin, mode || "stroke");
      })
      .catch(() => {
        const el = document.getElementById("hanziPracticeInfo");
        if (el)
          el.innerHTML =
            "<span>Không tải được dữ liệu nét.</span><span>Kiểm tra kết nối mạng.</span>";
      });
  }
  function clearStage() {
    ...
    const t = document.getElementById("hanziWriterTarget"),
      l = document.getElementById("hanziTraceLayer");
    if (t) t.innerHTML = "";
    if (l) l.innerHTML = "";
    const c = document.querySelector(".hanzi-draw-canvas");
    if (c) c.remove();
  }
  function setupMode(char, pinyin, mode) {
    document
      .querySelectorAll(".hanzi-mode-btn")
      .forEach((b) => b.classList.toggle("active", b.dataset.mode === mode));
    clearStage();
    if (mode === "trace") setupTrace(char);
    else if (mode === "quiz") setupQuiz(char);
    else setupStroke(char);
  }
  function makeWriter(char, opts) {
    const size = Math.min(
      360,
      document.getElementById("hanziWriterTarget").getBoundingClientRect()
        .width,
    );
    return window.HanziWriter.create(
      "hanziWriterTarget",
      char,
      Object.assign(
        {
          ...
          onLoadCharDataError: loadFailure,
        },
        opts || {},
      ),
    );
  ...
  function info(a, b) {
    const el = document.getElementById("hanziPracticeInfo");
    if (el) el.innerHTML = "<span>" + a + "</span><span>" + b + "</span>";
  }
  function setupStroke(char) {
    info("Đang phát thứ tự nét…", "Chạm “Xem nét” để xem lại");
    currentWriter = makeWriter(char, {
      showCharacter: false,
      showOutline: true,
    ...
    currentWriter
      .animateCharacter({
        onComplete: () => {
          info("✓ Hoàn tất thứ tự nét", "Có thể xem lại hoặc chuyển sang viết");
        },
      })
      .catch(loadFailure);
  }
  function setupTrace(char) {
    info("Viết đè theo chữ mờ", "Dùng chuột hoặc ngón tay để viết");
    currentWriter = makeWriter(char, {
      showCharacter: true,
      showOutline: false,
      strokeColor: "#b8ddca",
    ...
    const layer = document.getElementById("hanziTraceLayer");
    if (layer) layer.style.pointerEvents = "none";
    const canvas = document.createElement("canvas");
    canvas.className = "hanzi-draw-canvas";
    canvas.width = 720;
    canvas.height = 720;
    ...
    const down = (e) => {
      ...
      up = () => {
        drawing = false;
        if (pts) info("Đang viết mờ", "Tiếp tục luyện từng nét");
      };
  ...
  function setupQuiz(char) {
    info(
      "Tự viết · bắt đầu theo thứ tự nét",
      "Đưa ngón tay hoặc chuột vào ô chữ để viết",
    );
    ...
    currentWriter.quiz({
      ...
      onMistake: (d) =>
        info(
          "✗ Sai ở nét " + (d.strokeNum + 1),
          "Tổng lỗi: " + d.totalMistakes,
        ),
      onCorrectStroke: (d) =>
        info("✓ Đúng nét " + d.strokeNum, "Còn " + d.strokesRemaining + " nét"),
      onComplete: (d) =>
        info(
          "🎉 Hoàn thành chữ " + d.character,
          "Tổng lỗi: " + d.totalMistakes,
        ),
    });
  }
  ...
  function closeModal() {
    clearStage();
    const o = document.getElementById("hanziPracticeOverlay");
    if (o) o.remove();
    if (modalCleanup) {
      modalCleanup();
      modalCleanup = null;
  ...
  document.addEventListener("click", (e) => {
    const b = e.target.closest && e.target.closest(".hanzi-practice-btn");
    if (!b) return;
    e.preventDefault();
    openModal(
      b.dataset.char || "",
      b.dataset.pinyin || "",
      b.dataset.mode || "stroke",
    );
  });
