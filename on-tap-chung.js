...
function norm(l,u){
 ...
 return {id:l.id||u,href:u,bai:b,level:lv,title,vi:l.titleVi||l.viSubtitle||l.subtitle||'',vocab:v,grammar:g,questions:q}
}
...
function hanziPanel(){let p=$('panel-hanzi'),b=S.d.lessons.find(x=>x.id===S.lesson)?.bai,a=S.d.hanzi.filter(x=>(S.level==='all'||x.levels.includes(S.level))&&(S.lesson==='all'||x.bais.includes(b))&&(!S.q||(x.char+' '+x.words.join(' ')).toLowerCase().includes(S.q.toLowerCase())));p.innerHTML='<div class="card"><p>Chữ Hán được tổng hợp từ các từ vựng. Khi dữ liệu bài học có thêm trường <code>hanzi</code>, có thể mở rộng bộ thủ, cấu tạo, âm đọc và ghi chú.</p></div>'+(a.length?'<div class="hanzi-grid">'+a.map(x=>'<div class="item hanzi"><div class="char">'+esc(x.char)+'</div><div class="meta">Từ: '+esc(x.words.slice(0,4).join(' · '))+(x.words.length>4?' …':'')+'<br>'+esc(x.levels.join(' · '))+'</div></div>').join('')+'</div>':'<div class="empty">Không có chữ Hán phù hợp.</div>')}
