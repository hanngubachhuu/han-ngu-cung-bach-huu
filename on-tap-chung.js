(()=>{'use strict';
const S={d:{lessons:[],vocab:[],grammar:[],hanzi:[],questions:[]},tab:'overview',level:new URLSearchParams(location.search).get('level')==='1'?'HSK 1':'all',lesson:'all',q:'',fc:{items:[],i:0,flip:false,known:{}},exerciseLimit:100,quiz:{pool:[],i:0,a:{},done:false}};
...
function lvl(l){
  const n=Number(l?.content?.course?.level ?? l?.course?.level ?? l?.level);
  if(Number.isFinite(n)&&n>0)return 'HSK '+n;
  let s=[l?.title,l?.zhTitle,l?.titleVi,l?.viSubtitle,l?.subtitle,l?.heroTitle,...metaText(l)].filter(Boolean).join(' ');
  const m=s.match(/HSK\s*([1-9]\d?)/i);
  return m?'HSK '+m[1]:'Khác';
}
...
function norm(l,u){
 let b=bn(l,u),lv=lvl(l),c=l.content||{},title=l.zhTitle||l.title||c.course?.titleZh||u;
 let q=qs(l,b,lv);
 ...
 return {id:l.id||u,href:u,bai:b,level:lv,title,vi:l.titleVi||l.viSubtitle||l.subtitle||c.course?.titleVi||'',vocab:v,grammar:g,hanzi:hz,exampleSentences:c.exampleSentences||[],commonErrors:c.commonErrors||[],exerciseGroups:c.exercises||{},coverage:c.coverage||{},questions:q,content:c};
}
function agg(ls){
...
function stats(){const ls=S.d.lessons.filter(x=>(S.level==='all'||x.level===S.level)&&(S.lesson==='all'||x.id===S.lesson));const hs=new Set(ls.flatMap(x=>(x.hanzi||[]).map(h=>h.char)));[['statLessons',ls.length],['statVocab',S.d.vocab.filter(filt).length],['statGrammar',S.d.grammar.filter(filt).length],['statHanzi',hs.size],['statQuestions',S.d.questions.filter(filt).length]].forEach(([id,n])=>$(id).textContent=n)}
function filters(){let lv=uniq(S.d.lessons.map(x=>x.level),x=>x).sort();$('levelFilter').innerHTML='<option value="all">Tất cả cấp độ</option>'+lv.map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');let l=S.d.lessons.filter(x=>S.level==='all'||x.level===S.level);$('lessonFilter').innerHTML='<option value="all">Tất cả bài học</option>'+l.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.level+' · Bài '+x.bai+' · '+x.title)+'</option>').join('');$('lessonFilter').value=S.lesson;$('levelFilter').value=S.level}
function overview(){
  let p=$('panel-overview'),ls=S.d.lessons.filter(x=>(S.level==='all'||x.level===S.level)&&(S.lesson==='all'||x.id===S.lesson));
  p.innerHTML='<div class="card"><h2>Kho ôn tập dùng chung</h2><p>Phần này không còn thuộc riêng HSK 2. Nó gom từ vựng, ngữ pháp, chữ Hán và bài tập từ các bài học thành một kho chung.</p><p>Chọn cấp độ và bài học, lật thẻ từ để ôn nghĩa, hoặc mở bài tập để xem giải thích. Kiểm tra nhanh trộn 10 câu theo bộ lọc hiện tại.</p></div><div class="card"><h2>Các bài đang có</h2><div class="lesson-list">'+ls.map(x=>'<a class="lesson-link" href="'+esc(x.href)+'"><span><b>'+esc(x.level+' · Bài '+x.bai)+'</b></span><span>→</span></a>').join('')+'</div></div>'
}
function vocabPanel(){let p=$('panel-vocab'),a=S.d.vocab.filter(filt);S.fc.items=a;if(S.fc.i>=a.length)S.fc.i=0;p.innerHTML='<div class="action-row"><button class="chip active">Thẻ từ vựng</button><span class="mini" style="padding:9px 0">Mở từng từ để xem collocation, lưu ý sử dụng và phần mở rộng.</span></div>'+(a.length?'<div id="flash"></div><div class="vocab-grid">'+a.map(x=>'<div class="item"><span class="tag">'+esc(x.level+' · Bài '+x.bai)+'</span><div class="han">'+esc(x.han)+'</div><div class="pin">'+esc(x.pinyin||'')+'</div><div class="meaning">'+esc(x.nghia||x.meaning||'')+'</div><a href="tu-vung.html?word='+encodeURIComponent(x.han)+'&amp;lesson='+encodeURIComponent(x.lessonId)+'" style="display:inline-block;margin-top:9px;font-size:12px;font-weight:800">Tìm hiểu sâu từ này →</a></div>').join('')+'</div>':'<div class="empty">Không có từ phù hợp.</div>');flash()}
function flash(){let p=$('flash'),a=S.fc.items;if(!p||!a.length)return;let v=a[S.fc.i%a.length],k=v.level+'|'+v.bai+'|'+v.han;p.innerHTML='<div class="fc"><div class="fc-count">'+(S.fc.i+1)+' / '+a.length+'</div><div class="fc-card '+(S.fc.flip?'flipped':'')+'" id="fcCard"><div class="fc-front"><div class="han">'+esc(v.han)+'</div><div class="pin">'+esc(v.pinyin||'Chưa có pinyin')+'</div><div class="mini">Chạm để lật thẻ</div></div><div class="fc-back"><div class="meaning">'+esc(v.nghia||v.meaning||'Chưa có nghĩa chuẩn hóa')+'</div><div class="mini">'+esc(v.level+' · Bài '+v.bai+' · '+v.sourceTitle)+'</div></div></div><div class="fc-actions"><button class="btn" id="fp">←</button><button class="btn btn-green" id="fk">'+(S.fc.known[k]?'✓ Đã nhớ':'✓ Đánh dấu đã nhớ')+'</button><button class="btn btn-primary" id="fn">→</button></div></div>';$('fcCard').onclick=()=>{S.fc.flip=!S.fc.flip;flash()};$('fp').onclick=()=>{S.fc.i=(S.fc.i-1+a.length)%a.length;S.fc.flip=false;flash()};$('fn').onclick=()=>{S.fc.i=(S.fc.i+1)%a.length;S.fc.flip=false;flash()};$('fk').onclick=()=>{S.fc.known[k]=!S.fc.known[k];localStorage.setItem('common_review_known',JSON.stringify(S.fc.known));flash()}}
function grammarPanel(){let p=$('panel-grammar'),a=S.d.grammar.filter(filt);p.innerHTML=a.length?a.map(x=>'<div class="item" style="margin-bottom:10px"><span class="tag">'+esc(x.level+' · Bài '+x.bai)+'</span><h3>'+esc(x.title)+'</h3><p>'+esc(x.explanation||x.desc||'')+'</p>'+((x.pattern||x.examples?.length)?'<div style="background:var(--primary-soft);padding:9px;border-radius:8px">'+(x.pattern?'<b>'+esc(x.pattern)+'</b><br>':'')+esc((x.examples||[]).join(' · '))+'</div>':'')+'</div>').join(''):'<div class="empty">Chưa có điểm ngữ pháp phù hợp.</div>'}
function hanziPanel(){
 let p=$('panel-hanzi'),selected=S.d.lessons.find(x=>x.id===S.lesson),b=selected?.bai;
 let a=S.d.hanzi.filter(x=>(S.level==='all'||x.levels.includes(S.level))&&(S.lesson==='all'||(x.bais.includes(b)&&x.levels.includes(selected?.level)))&&(!S.q||(x.char+' '+x.words.join(' ')+' '+(x.notes||'')).toLowerCase().includes(S.q.toLowerCase())));
...
function ans(q){if(q.model||q.modelAnswer)return q.model||q.modelAnswer;if(q.type==='mcq'||q.type==='listening')return((q.options||[]).find(x=>String(x.k)===String(q.answer))||{}).t||String(q.answer||'');if(Array.isArray(q.answer))return q.answer.join(' / ');if(q.correctMap)return JSON.stringify(q.correctMap);return String(q.answer||'')}
function exercisePanel(){let p=$('panel-exercises'),all=S.d.questions.filter(filt),a=all.slice(0,S.exerciseLimit);p.innerHTML=a.length?'<div class="card"><p>Ngân hàng này lấy câu hỏi trực tiếp từ các bài học. Các dạng mở, dịch, giao tiếp hoặc chưa có bộ chấm riêng vẫn được giữ lại để xem đề và giải thích.</p></div>'+a.map(x=>'<div class="exercise"><div><span class="tag">'+esc(x.level+' · Bài '+x.bai)+'</span><span class="tag">'+esc(x.skill)+'</span></div><div class="prompt">'+esc([x.readingText,x.hint,x.prompt||x.title].filter(Boolean).join('\n'))+'</div>'+((x.type==='mcq'||x.type==='listening')?'<div class="review-options">'+(x.options||[]).map(o=>'<p>'+esc(o.k+'. '+o.t)+'</p>').join('')+'</div>':'')+(x.audioStatus==='ready'&&x.audioSrc?'<audio controls preload="none" src="'+esc(x.audioSrc)+'" aria-label="Audio bài nghe"></audio>':'')+(x.sourceHref?'<p><a href="'+esc(x.sourceHref)+'">Mở bài học →</a></p>':'')+'<button class="btn" type="button">Xem đáp án / giải thích</button><div class="answer"><b>Đáp án:</b> '+esc(ans(x))+((x.explanation||x.explain)?'<br><br>'+esc(String(x.explanation||x.explain).replace(/<[^>]+>/g,' ')):'')+'</div></div>').join(''):'<div class="empty">Không có bài tập phù hợp.</div>';p.querySelectorAll('.exercise button').forEach(b=>b.onclick=()=>b.parentElement.classList.toggle('open'));if(all.length>a.length){const more=document.createElement('button');more.className='btn btn-primary';more.textContent='Xem thêm 100 câu ('+a.length+'/'+all.length+')';more.onclick=()=>{S.exerciseLimit+=100;exercisePanel();};p.appendChild(more)}}
...
function qview(){let p=$('panel-quiz'),q=S.quiz.pool[S.quiz.i];if(!q)return qresult();let h='<div class="card quiz"><div class="mini">Câu '+(S.quiz.i+1)+' / '+S.quiz.pool.length+' · '+esc(q.level+' · Bài '+q.bai)+' · '+esc(q.skill)+'</div><h2>'+esc([q.readingText,q.hint,q.prompt].filter(Boolean).join('\n'))+'</h2>';if(q.type==='mcq')h+=(q.options||[]).map(o=>'<button class="quiz-opt '+(S.quiz.a[S.quiz.i]===o.k?'selected':'')+'" data-k="'+esc(o.k)+'">'+esc(o.k+'. '+o.t)+'</button>').join('');else h+='<input id="qi" class="quiz-input" value="'+esc(S.quiz.a[S.quiz.i]||'')+'" placeholder="Nhập câu trả lời…">';h+='<div class="action-row"><button class="btn btn-primary" id="qn">'+(S.quiz.i===S.quiz.pool.length-1?'Nộp bài':'Câu tiếp theo →')+'</button></div></div>';p.innerHTML=h;p.querySelectorAll('.quiz-opt').forEach(b=>b.onclick=()=>{S.quiz.a[S.quiz.i]=b.dataset.k;p.querySelectorAll('.quiz-opt').forEach(x=>x.classList.remove('selected'));b.classList.add('selected')});let inp=$('qi');if(inp)inp.oninput=()=>S.quiz.a[S.quiz.i]=inp.value;$('qn').onclick=()=>{if(S.quiz.i<S.quiz.pool.length-1){S.quiz.i++;qview()}else{S.quiz.done=true;qresult()}}}
function grade(q,a){if(a==null)return false;if(q.type==='mcq')return String(a)===String(q.answer);if(q.type==='text_fill'){let n=s=>q.level==='HSK 1'?String(s||'').normalize('NFC').trim().replace(/[\s。.!！?？]/g,''):String(s||'').trim().toLowerCase();return(Array.isArray(q.answer)?q.answer:[q.answer]).some(x=>n(x)===n(a))}return false}
function qresult(){let c=0;S.quiz.pool.forEach((q,i)=>{if(grade(q,S.quiz.a[i]))c++});let p=$('panel-quiz'),pct=S.quiz.pool.length?Math.round(c*100/S.quiz.pool.length):0;p.innerHTML='<div class="result"><b>'+pct+'%</b><div>Đúng '+c+'/'+S.quiz.pool.length+' câu</div></div>'+S.quiz.pool.map((q,i)=>'<div class="exercise"><b>'+(grade(q,S.quiz.a[i])?'✓ Đúng':'✗ Chưa đúng')+' · Câu '+(i+1)+'</b><p class="prompt">'+esc([q.readingText,q.prompt].filter(Boolean).join('\n'))+'</p><p><b>Đáp án:</b> '+esc(ans(q))+'</p><div style="white-space:pre-line">'+esc(q.explain||q.explanation||'')+'</div></div>').join('')+'<button class="btn" id="retry">Làm lượt mới</button>';$('retry').onclick=quizSetup}
function render(){stats();overview();vocabPanel();grammarPanel();hanziPanel();exercisePanel();if(S.tab==='quiz')quizSetup()}
...
function bind(){
 ...
 $('search').oninput=e=>{S.q=e.target.value.trim();render()};$('levelFilter').onchange=e=>{S.level=e.target.value;S.lesson='all';S.exerciseLimit=100;filters();render()};$('lessonFilter').onchange=e=>{S.lesson=e.target.value;S.exerciseLimit=100;render()};
...
async function load(){
 ...
 S.d=agg(ls);filters();stats();render();if(S.level==='HSK 1'){document.title='Ôn tập HSK 1 — Hán Ngữ Cùng Bách Hữu';}
