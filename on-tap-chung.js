...
function overview(){
  let p=$('panel-overview'),ls=S.d.lessons.filter(x=>S.level==='all'||x.level);
  p.innerHTML='<div class="card"><h2>Kho ôn tập dùng chung</h2><p>Phần này không còn thuộc riêng HSK 2. Nó gom từ vựng, ngữ pháp, chữ Hán và bài tập từ các bài học thành một kho chung.</p><p><b>Luồng:</b> Bài học → dữ liệu LESSON → Ôn tập chung. Khi bài mới được thêm vào lộ trình trên trang chủ, hệ thống tự phát hiện và đọc dữ liệu bài đó.</p></div><div class="card"><h2>Các bài đang có</h2><div class="lesson-list">'+ls.map(x=>'<a class="lesson-link" href="'+esc(x.href)+'"><span><b>'+esc(x.level+' · Bài '+x.bai)+'</b></span><span>→</span></a>').join('')+'</div></div>'
}
function vocabPanel(){let p=$('panel-vocab'),a=S.d.vocab.filter(filt);S.fc.items=a;if(S.fc.i>=a.length)S.fc.i=0;p.innerHTML='<div class="action-row"><button class="chip active">Thẻ từ vựng</button><span class="mini" style="padding:9px 0">Mở từng từ để xem collocation, lưu ý sử dụng và phần mở rộng.</span></div>'+(a.length?'<div id="flash"></div><div class="vocab-grid">'+a.map(x=>'<div class="item"><span class="tag">'+esc(x.level+' · Bài '+x.bai)+'</span><div class="han">'+esc(x.han)+'</div><div class="pin">'+esc(x.pinyin||'')+'</div><div class="meaning">'+esc(x.nghia||x.meaning||'')+'</div><a href="tu-vung.html?word='+encodeURIComponent(x.han)+'" style="display:inline-block;margin-top:9px;font-size:12px;font-weight:800">Tìm hiểu sâu từ này →</a></div>').join('')+'</div>':'<div class="empty">Không có từ phù hợp.</div>');flash()}
...
function hanziPanel(){
 let p=$('panel-hanzi'),b=S.d.lessons.find(x=>x.id===S.lesson)?.bai;
 let a=S.d.hanzi.filter(x=>(S.level==='all'||x.levels.includes(S.level))&&(S.lesson==='all'||x.bais.includes(b))&&(!S.q||(x.char+' '+x.words.join(' ')+' '+(x.notes||'')).toLowerCase().includes(S.q.toLowerCase())));
...
function exercisePanel(){let p=$('panel-exercises'),a=S.d.questions.filter(filt).slice(0,100);p.innerHTML=a.length?'<div class="card"><p>Ngân hàng này lấy câu hỏi trực tiếp từ các bài học. Các dạng mở, dịch, giao tiếp hoặc chưa có bộ chấm riêng vẫn được giữ lại để xem đề và giải thích.</p></div>'+a.map(x=>'<div class="exercise"><div><span class="tag">'+esc(x.level+' · Bài '+x.bai)+'</span><span class="tag">'+esc(x.skill)+'</span></div><div class="prompt">'+esc(x.prompt||x.title||'')+'</div><button class="btn" type="button">Xem đáp án / giải thích</button><div class="answer"><b>Đáp án:</b> '+esc(ans(x))+((x.explanation||x.explain)?'<br><br>'+esc(String(x.explanation||x.explain).replace(/<[^>]+>/g,' ')):'')+'</div></div>').join(''):'<div class="empty">Không có bài tập phù hợp.</div>';p.querySelectorAll('.exercise button').forEach(b=>b.onclick=()=>b.parentElement.classList.toggle('open'))}
