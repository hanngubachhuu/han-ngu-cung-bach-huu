...
function hanziPanel(){let p=$('panel-hanzi'),b=S.d.lessons.find(x=>x.id===S.lesson)?.bai,a=S.d.hanzi.filter(x=>(S.level==='all'||x.levels.includes(S.level))&&(S.lesson==='all'||x.bais.includes(b))&&(!S.q||(x.char+' '+x.words.join(' ')+' '+(x.notes||'')).toLowerCase().includes(S.q.toLowerCase())));p.innerHTML='<div class="card"><p>Chữ Hán được tổng hợp từ từ vựng. Nếu bài học có dữ liệu <code>hanzi</code> riêng, các trường như bộ thủ, cấu tạo, âm đọc và ghi chú sẽ được giữ nguyên.</p></div>'+(a.length?'<div class="hanzi-grid">'+a.map(x=>'<div class="item hanzi"><div class="char">'+esc(x.char)+'</div>'+(x.pinyin?'<div class="pin">'+esc(x.pinyin)+'</div>':'')+(x.meaning?'<div class="meaning">'+esc(x.meaning)+'</div>':'')+(x.radical||x.structure?'<div class="meta">'+esc([x.radical?'Bộ: '+x.radical:'',x.structure?'Cấu tạo: '+x.structure:''].filter(Boolean).join(' · '))+'</div>':'')+'<div class="meta">Từ: '+esc(x.words?.slice(0,4).join(' · ')||'')+(x.words&&x.words.length>4?' …':'')+'<br>'+esc(x.levels.join(' · '))+'</div>'+(x.notes?'<div class="meta" style="margin-top:6px">'+esc(x.notes)+'</div>':'')+'</div>').join('')+'</div>':'<div class="empty">Không có chữ Hán phù hợp.</div>')}
